...
if (!class_exists('WmsUpdate')) {
    ...
    class WmsUpdate
    {
                                    ...
                                    $type = 'theme', $plugin_file = '')
        {
            ...
            add_action('wdc_admin_page_before', array($this, 'wms_show_admin_notices'));
        }

        public function wms_show_admin_notices()
        {
            $options = get_option('wms_settings_auth');

            if (!$options || !isset($options['wms_license_email']) || !isset($options['wms_license_key']) ||
                $options['wms_license_email'] == '' || $options['wms_license_key'] == '') {

                $msg = __('Пожалуйста введите ваш Email и Лицензионый ключ для доступа к обновлениям и тех. помощи плагина </br><i> %s</i>.', $this->text_domain);
                $msg = sprintf($msg, $this->product_name);
                ?>
                <div class="update-nag alert alert-danger">
                    <p>
                        <?php echo $msg; ?>

                    </p>

                    <?php if (!isset($_REQUEST['page']) || $_REQUEST['page'] !== 'wms-settings-page') { ?>
                        <p>
                            <a href="<?php echo admin_url('?page=wms-settings-page'); ?>">
                                <?php _e('Перейти на страницу настроек.', $this->text_domain); ?>
                            </a>
                        </p>
                    <?php } ?>
            ...
            if ($this->is_update_available()) {
                ?>
                <div class="notice notice-info is-dismissible " style="padding: 10px">
                    <p>
                        Для плагина Интеграция МойСклад и WooCommerce доступно обновление
                    </p>
                    <p>
                        <a href="<?php echo admin_url('update-core.php?force-check=1'); ?>">
                            <?php _e('Перейти на страницу обновления.', $this->text_domain); ?>
                        </a>
                    </p>
                    <p>
                        Если обновление не появилось в списке, нажать кнопку прверить обновления
                    </p>
                </div>
                <?php
            }
