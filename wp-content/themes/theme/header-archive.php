...
<? if ( is_user_logged_in() ) {
    ...
    if($row == '') { unset($row); }
    if (isset($row)) {
        if ($row == 1) { echo 'РЎР°РјРѕРІС‹РІРѕР·'; $resultArray = get_user_meta( $user_id, 'billing_samoviziv', true ); }
        if ($row == 0) {
    ...
    } else { echo 'Р”РѕСЃС‚Р°РІРєР° РёР»Рё СЃР°РјРѕРІС‹РІРѕР·'; $resultArray = 'Р’С‹Р±РµСЂРёС‚Рµ СЃРїРѕСЃРѕР± РїРѕР»СѓС‡РµРЅРёСЏ'; }
} else {
    $row = $_COOKIE['delivery'];
    if (isset($row)) {
        if ($row == 1) { echo 'РЎР°РјРѕРІС‹РІРѕР·'; $resultArray = $_COOKIE['billing_samoviziv']; }
        if ($row == 0) {
            if(isset($_COOKIE['delivery_time']) && isset($_COOKIE['delivery_day'])) {
                if($_COOKIE['delivery_day'] == "today") { echo ''; }
                else if($_COOKIE['delivery_day'] == "tomorrow") { echo ''; }
    ...
    <div class="tab-container1">
        ...
        <div id="tab1" class="modal-delivery__tab tab-content1 active1">
            ...
            <div class="VV_RWayChoiceModalDR__Note" id="delivery-message" style="display: none; gap: 8px; color: #c31611; margin-bottom:0rem">
                ...
                <div class="VV_RWayChoiceModalDR__NoteCol _text">
                    <span class="VV_RWayChoiceModalDR__NoteText rtext _desktop-sm _tablet-sm _mobile-sm">Рљ СЃРѕР¶Р°Р»РµРЅРёСЋ, РґРѕСЃС‚Р°РІРєР° РЅРµ СЂР°Р±РѕС‚Р°РµС‚ РІ РІС‹Р±СЂР°РЅРЅРѕРј РјРµСЃС‚Рµ.</span>
                </div>
            </div>
            ...
            <div id="time_change">
                ...
                <div class="modal-delivery__time flex_time" id="flex_time" style="">
                    <div class="modal-delivery__time-item mainblock_time <?if ( $current_time > '14:00' ) {echo 'notenable';}?>" style="display:none" data-count='1' data-time='1'>
                        <div class="underblocktime"><p class="delivery-text">Р”РѕСЃС‚Р°РІРєР°</p><p>15-17</p><p>РЎРµРіРѕРґРЅСЏ</p></div>
                    </div>
                    ...
                    <div class="modal-delivery__time-item mainblock_time <?if ( $current_time > '20:00' ) {echo 'notenable';}?>" style="display:none" data-count='2' data-time='1'>
                        <div class="underblocktime"><p class="delivery-text">Р”РѕСЃС‚Р°РІРєР°</p><p>19-22</p><p>РЎРµРіРѕРґРЅСЏ</p></div>
                    </div>
                    <div class="modal-delivery__time-item mainblock_time" style="display:none" data-count='1' data-time='2'>
                        <div class="underblocktime"><p class="delivery-text">Р”РѕСЃС‚Р°РІРєР°</p><p>15-17</p><p>Р—Р°РІС‚СЂР°</p></div>
                    </div>
                    <div class="modal-delivery__time-item mainblock_time" style="display:none" data-count='2' data-time='2'>
                        <div class="underblocktime"><p class="delivery-text">Р”РѕСЃС‚Р°РІРєР°</p><p>19-22</p><p>Р—Р°РІС‚СЂР°</p></div>
                    </div>
                </div>
                <script>
            ...
            <input type="text" id="comment_cur" placeholder="РљРѕРјРјРµРЅС‚Р°СЂРёР№ РєСѓСЂСЊРµСЂСѓ" style="width: 100%; margin-top: 1em; display:none; border-radius: 10px; background: #f8f8fa; height: 47px; padding-left: 15px;">
            <input type="text" id="comment_cur1" placeholder="РљРѕРјРјРµРЅС‚Р°СЂРёР№ РїРѕ Р·Р°РєР°Р·Сѓ" style="width: 100%; margin-top: 1em; display:none; border-radius: 10px; background: #f8f8fa; height: 47px; padding-left: 15px;">
            ...
            <script>
                ...
                function init() {
                        ...
                        ymaps.geocode([lat, lon]).then(function (res) {
                            ...
                            if(res.geoObjects.get(0).getPremiseNumber() != null) {
                                jQuery.ajax({
                                        ...
                                        if(!data.success) {
                                            ...
                                            if(typeof data.data.today != "undefined") {
                                                $.each(data.data.today, function(type, price) {
                                                    if(price.price == 0) { price.price = 'Р‘РµСЃРїР»Р°С‚РЅРѕ'; }
                                                    else { price.price = 'РѕС‚ 0 РґРѕ ' + price.price + '&nbsp;в‚Ѕ'; }
                                                    let checked = '';
                                                    if(data.data.current == 'today_' + type) { checked = ' enable'; }
                                                    choices += '<div class="mainblock_time' + checked + '" data-day="today" data-time="' + type + '"><div class="underblocktime"><p class="delivery-text">РЎРµРіРѕРґРЅСЏ</p><p>' + price.description + '</p><p class="mainblock_time__price">' + price.price + '</p></div></div>';
                                                });
                                            }
                                            if(typeof data.data.tomorrow != "undefined") {
                                                $.each(data.data.tomorrow, function(type,price) {
                                                    ...
                                                    choices += '<div class="mainblock_time' + checked + '" data-day="tomorrow" data-time="' + type + '"><div class="underblocktime"><p class="delivery-text">Р—Р°РІС‚СЂР°</p><p>' + price.description + '</p><p class="mainblock_time__price">' + price.price + '</p></div></div>';
                                                });
                                            }
                                    ...
                                    error: function () { deliveryMessage.innerHTML = 'РћС€РёР±РєР°'; deliveryMessage.style.display = 'flex'; }
                                });
                            }
                        });
                    }
                    ...
                    function createPlacemark(coords) {
                        return new ymaps.Placemark(coords, { iconCaption: 'РїРѕРёСЃРє...' }, { preset: 'islands#violetDotIconWithCaption', draggable: true });
                    }
        ...
        <div id="tab2" class="modal-delivery__tab tab-content1">
            <button id="list_market" class="modal-delivery__tab-shop VV_RWayModalMap__ListShopsShower js-delivery__shopselect--search-shops-list">
                <span class="VV_RWayModalMap__ListShopsShowerText b700">РЎРїРёСЃРѕРє РјР°РіР°Р·РёРЅРѕРІ</span>
            </button>
            ...
            <div class="modal-delivery__tab-selects" style="display:flex;">
                <div class="first_part_time" style="width:27%;">
                    <select class="modal-delivery__tab-select" name="data" id="id_select_time_samoviviz">
                        ...
                        <option value="tomorrow" <?if ($current_time1 > '20:00') {echo 'selected';}?>>Р—Р°РІС‚СЂР°</option>
                    </select>
                </div>
            ...
            <button type="button" id="but_dev2" class="modal-delivery__choice disabled-btn">Р’С‹Р±СЂР°С‚СЊ СЃР°РјРѕРІС‹РІРѕР·</button>
        </div>
    </div>
</div>
</div>
...
<div class="modal-content modal-delivery__selfpickup">
    ...
    <div id="market_egersheld" class="market_el">
        ...
        <div class="mainblock_time1 enable1" data-market="11"><div class="underblocktime1"><p class="delivery-text modal-delivery__selfpickup-btn" style="margin-bottom:0px;">Р’С‹Р±СЂР°С‚СЊ</p></div></div>
    </div>
    <script>
    ...
    <div id="market_remicity" class="market_el">
        ...
        <div class="mainblock_time1 enable1" data-market="1"><div class="underblocktime1"><p class="delivery-text modal-delivery__selfpickup-btn" style="margin-bottom:0px;">Р’С‹Р±СЂР°С‚СЊ</p></div></div>
    </div>
    <div id="market_zarya" class="market_el">
        <p class="modal-delivery__selfpickup-adress">Р—Р°СЂСЏ (СѓР». Р§РєР°Р»РѕРІР°, 30)</p>
        <div class="mainblock_time1 enable1" data-market="6"><div class="underblocktime1"><p class="delivery-text modal-delivery__selfpickup-btn" style="margin-bottom:0px;">Р’С‹Р±СЂР°С‚СЊ</p></div></div>
    </div>
    <div id="market_sputnik" class="market_el">
        ...
        <div class="mainblock_time1 enable1" data-market="3"><div class="underblocktime1"><p class="delivery-text modal-delivery__selfpickup-btn" style="margin-bottom:0px;">Р’С‹Р±СЂР°С‚СЊ</p></div></div>
    </div>
</div>
</div>
<style>
...
function init() {
    ...
    var myPlacemark11 = new ymaps.Placemark([43.09968, 131.863907], { hintContent: 'Р­РіРµСЂС€РµР»СЊРґ, Р’РµСЂС…РЅРµРїРѕСЂС‚РѕРІР°СЏ,68Р°' }, { balloonContentLayout: null });
    var myPlacemark1 = new ymaps.Placemark([43.128381, 131.919746], { hintContent: 'Р РµРјРё-РЎРёС‚Рё (СѓР». РќР°СЂРѕРґРЅС‹Р№ РїСЂ-С‚, 20)' }, { balloonContentLayout: null });
    var myPlacemark3 = new ymaps.Placemark([43.24827778336888, 132.02109573106299], { hintContent: 'СѓР». РўРёРјРёСЂСЏР·РµРІР°,31 СЃС‚СЂРѕРµРЅРёРµ 1 (СЂР°Р№РѕРЅ РЎРїСѓС‚РЅРёРє)' }, { balloonContentLayout: null });
    var myPlacemark6 = new ymaps.Placemark([43.181235883133674,131.9154298472213], { hintContent: 'Р—Р°СЂСЏ (Р§РєР°Р»РѕРІР°, 30)' }, { balloonContentLayout: null });
    var myPlacemark8 = new ymaps.Placemark([43.132657, 131.905418], { hintContent: 'РћРєРµР°РЅСЃРєРёР№ РїСЂРѕСЃРїРµРєС‚ 108' }, { balloonContentLayout: null });
    ...
    myGroup.events.add('click', function (e) {
        ...
        ymaps.geocode(target.geometry.getCoordinates()).then(function (res) {
            ...
            if (pickupHint) { pickupHint.innerHTML = 'рџ“Ќ Р’С‹ РІС‹Р±СЂР°Р»Рё: <b>' + target.properties.get('hintContent') + '</b>'; pickupHint.style.color = '#4FBD01'; }
        });
    });

    /* FIX: РјР°РїРїРёРЅРі data-market в†’ placemark, СѓР±СЂР°РЅС‹ РЅРµСЃСѓС‰РµСЃС‚РІСѓСЋС‰РёРµ РјРµС‚РєРё */
    ...
    document.querySelectorAll('.mainblock_time1').forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            if (pickupHint && selectedName) { pickupHint.innerHTML = 'рџ“Ќ Р’С‹ РІС‹Р±СЂР°Р»Рё: <b>' + selectedName + '</b>'; pickupHint.style.color = '#4FBD01'; }
        });
    });
}
</script>
...
<script>
/* FIX: РѕР±СЂР°Р±РѕС‚С‡РёРє but_dev вЂ” Р·Р°С‰РёС‚Р° РѕС‚ null + РєРѕСЂСЂРµРєС‚РЅС‹Рµ Р°С‚СЂРёР±СѓС‚С‹ */
...
<?php
/* Р’СЃРїРѕРјРѕРіР°С‚РµР»СЊРЅР°СЏ С„СѓРЅРєС†РёСЏ РґР»СЏ РєРЅРѕРїРєРё РґРѕСЃС‚Р°РІРєРё вЂ” СѓР±РёСЂР°РµС‚ РґСѓР±Р»РёСЂРѕРІР°РЅРёРµ РєРѕРґР°.
FIX: echo ':' Р·Р°РјРµРЅРµРЅРѕ РЅР° echo '' */
...
<button class="header__delivery btn-grey open-modal1 open-modal__st">
    <? if ( is_user_logged_in() ) {
        ...
        } else { $resultArray = 'Р’С‹Р±РµСЂРёС‚Рµ СЃРїРѕСЃРѕР± РїРѕР»СѓС‡РµРЅРёСЏ'; }
    } else {
        $row = $_COOKIE['delivery'];
        if (isset($row)) {
            if ($row == 1) { echo 'РЎР°РјРѕРІС‹РІРѕР·:'; $resultArray = $_COOKIE['billing_samoviziv']; }
        ...
        <div class="container">
            <div class="header__follow-inner">
                ...
                <a class="header__logo" href="<? echo get_home_url(); ?>">
                    ...
                    <div><p>Р¤Р•Р РњРђ DР’</p><span>С„РµСЂРјРµСЂСЃРєРёР№ РјР°РіР°Р·РёРЅ</span></div>
                </a>
                <div class="header__desktop-menu">
                    <nav><ul>
                            <li><a href="<? echo get_home_url(); ?>/about/">Рћ РЅР°СЃ</a></li>
                            <li><a href="<? echo get_home_url(); ?>/blog/">Р‘Р»РѕРі</a></li>
                            <li><a href="<? echo get_home_url(); ?>/recipes/">Р РµС†РµРїС‚С‹</a></li>
                            <li><a href="<? echo get_home_url(); ?>/dostavka/">Р”РѕСЃС‚Р°РІРєР° Рё РѕРїР»Р°С‚Р°</a></li>
                            <li><a class="text-red" href="<? echo get_home_url(); ?>/stock/">РђРєС†РёРё</a></li>
                            <li><a href="<? echo get_home_url(); ?>/nashi-magaziny/">РќР°С€Рё РјР°РіР°Р·РёРЅС‹</a></li>
                            <li><a href="<? echo get_home_url(); ?>/bonusnaya-programma/">Р‘РѕРЅСѓСЃС‹</a></li>
                            <li><a href="<? echo get_home_url(); ?>/horeca/">HoReCa</a></li>
                        </ul></nav>
                </div>
                <div class="header__desktop-bot">
                    <button class="header__catalog btn-green" id="catalog-follow-btn"><img src="<?php bloginfo('template_url') ?>/assets/img/menu.svg" alt="РљР°С‚Р°Р»РѕРі"></button>
        ...
        <div class="header__inner">
            <div class="header__desktop">
                <div class="header__desktop-top">
                    <a class="header__logo" href="<? echo get_home_url(); ?>"><img src="<?php bloginfo('template_url') ?>/assets/img/logo.svg" alt=""><div><p>Р¤Р•Р РњРђ DР’</p><span>С„РµСЂРјРµСЂСЃРєРёР№ РјР°РіР°Р·РёРЅ</span></div></a>
                ...
                <div class="header__tablet-top">
                    ...
                    <div class="header__tablet-buttons">
                        <button class="header__cart cart-btn btn-grey mob_hidden"><span class="cart-count"><?php echo count( WC()->cart->get_cart() ); ?></span><img src="<?php bloginfo('template_url') ?>/assets/img/cart.svg" alt="РљРѕСЂР·РёРЅР°"></button>
                        <a class="header__bonus btn-grey mob_hidden" href="<? echo get_home_url(); ?>/bonusnaya-programma/"><img src="<?php bloginfo('template_url') ?>/assets/img/cow.svg" alt="Р‘РѕРЅСѓСЃС‹"><span><?php echo esc_html( do_shortcode( '[user_bonus_count]' ) ); ?></span></a>
                        <a class="header__profile btn-grey mob_hidden" href="<? echo get_home_url(); ?>/my-account/"><img src="<?php bloginfo('template_url') ?>/assets/img/profile.svg" alt="РџСЂРѕС„РёР»СЊ"></a>
                    </div>
                ...
                <div class="header__mobile">
                    ...
                    <div class="header__mobile-top">
                        <a class="header__phone btn-grey" href="tel:+79084411110"><p>+7-908-441-1110</p><img src="<?php bloginfo('template_url') ?>/assets/img/phone_2.svg" alt="РўРµР»РµС„РѕРЅ"></a>
                        <a class="header__bonus btn-grey mob_hidden" href="<? echo get_home_url(); ?>/bonusnaya-programma/"><img src="<?php bloginfo('template_url') ?>/assets/img/cow.svg" alt="Р‘РѕРЅСѓСЃС‹"><span><?php echo esc_html( do_shortcode( '[user_bonus_count]' ) ); ?></span></a>
                        <a class="header__profile btn-grey mob_hidden" href="<? echo get_home_url(); ?>/my-account/"><img src="<?php bloginfo('template_url') ?>/assets/img/profile.svg" alt="РџСЂРѕС„РёР»СЊ"></a>
                        <button class="header__profile btn-grey" id="open-search"><img src="<?php bloginfo('template_url') ?>/assets/img/search.svg" alt="РџРѕРёСЃРє"></button>
                    </div>
                    <div class="header__mobile-banner"><div class="swiper bannerSwiper"><div class="swiper-wrapper">
                                <div class="mobile__banner-item swiper-slide"><img src="<?php bloginfo('template_url') ?>/assets/img/banner_sale.svg" alt="РњРѕР»РѕС‡РЅР°СЏ РїСЂРѕРґСѓРєС†РёСЏ"><p>РњРѕР»РѕС‡РЅС‹Р№ РїРѕРЅРµРґРµР»СЊРЅРёРє - СЃРєРёРґРєР° 10% РЅР° РњРѕР»РѕС‡РєСѓ</p></div>
                                <div class="mobile__banner-item swiper-slide"><img src="<?php bloginfo('template_url') ?>/assets/img/myaso-akcii.svg" alt="Р¤РµСЂРјР° Р”Р’"><p>РњСЏСЃРЅРѕРµ РІРѕСЃРєСЂРµСЃРµРЅСЊРµ - СЃРєРёРґРєР° 10% РЅР° РІСЃСЋ РєР°С‚РµРіРѕСЂРёСЋ РњСЏСЃРѕ</p></div>
                ...
                <div class="header__desktop-bot">
                    <button class="header__catalog btn-green" id="catalog-desc-btn"><img src="<?php bloginfo('template_url') ?>/assets/img/menu.svg" alt="РљР°С‚Р°Р»РѕРі">РљР°С‚Р°Р»РѕРі</button>
            ...
            <div class="mobile__nav">
                <button class="mobile__nav-menu" id="mob-menu"><div class="mobile__nav-menu--line"><div></div><div></div><div></div></div>РјРµРЅСЋ</button>
                <button type="button" class="header__catalog" onclick="window.location.href='<?php echo get_home_url(); ?>/stock/';"><img src="<?php bloginfo('template_url') ?>/assets/img/discount.svg" alt="Р°РєС†РёРё">Р°РєС†РёРё</button>
                <button class="header__catalog" id="catalog-mob"><img src="<?php bloginfo('template_url') ?>/assets/img/catalog.svg" alt="РљР°С‚Р°Р»РѕРі">РєР°С‚Р°Р»РѕРі</button>
                <button class="header__catalog" id="to-open-contacts"><img src="<?php bloginfo('template_url') ?>/assets/img/phone.svg" alt="РєРѕРЅС‚Р°РєС‚С‹">РєРѕРЅС‚Р°РєС‚С‹</button>
                <button class="header__cart cart-btn"><span class="cart-count"><?php echo count( WC()->cart->get_cart() ); ?></span><img src="<?php bloginfo('template_url') ?>/assets/img/cart.svg" alt="РљРѕСЂР·РёРЅР°">РєРѕСЂР·РёРЅР°</button>
            </div>
            <div class="mob-menu" id="mob-menu-container"><nav>
                    <a class="mob-menu__title" href="<? echo get_home_url(); ?>"><p>Р¤Р•Р РњРђ DР’</p><span>С„РµСЂРјРµСЂСЃРєРёР№ РјР°РіР°Р·РёРЅ</span></a>
                    <ul class="mob-menu__list">
                        <li class="mob-menu__list-item"><a href="<? echo get_home_url(); ?>/">Р“Р»Р°РІРЅР°СЏ</a></li>
                        <li class="mob-menu__list-item"><a href="<? echo get_home_url(); ?>/about/">Рћ РЅР°СЃ</a></li>
                        <li><a href="<? echo get_home_url(); ?>/blog/">Р‘Р»РѕРі</a></li>
                        <li><a href="<? echo get_home_url(); ?>/recipes/">Р РµС†РµРїС‚С‹</a></li>
                        <li><a href="<? echo get_home_url(); ?>/dostavka/">Р”РѕСЃС‚Р°РІРєР° Рё РѕРїР»Р°С‚Р°</a></li>
                        <li><a class="text-red" href="<? echo get_home_url(); ?>/stock/">РђРєС†РёРё</a></li>
