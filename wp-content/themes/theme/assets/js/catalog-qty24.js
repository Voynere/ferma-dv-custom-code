...
jQuery(function($){
    ...
    function disablePromoGiftQty() {
        $('.cart__qty[data-promo_gift="1"]').each(function() {
            $(this).find('.cart__qty-btn--plus')
                .addClass('is-disabled')
                .prop('disabled', true);

            $(this).find('.cart__qty-btn--minus')
                .addClass('is-disabled')
                .prop('disabled', true);
        });
    }
    ...
    // при каждом обновлении мини-корзины
    $(document.body).on('wc_fragments_refreshed updated_wc_div updated_cart_totals update_checkout', function(){
        disablePromoGiftQty();
    });
