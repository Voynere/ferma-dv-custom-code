...
<head>
	...
	<link rel='stylesheet' href='https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v4.7.0/mapbox-gl-geo|coder.min.css' type='text/css' />
...
<!-- Старый код 3 -->
    ...
    <div class="modal1">
        <div class="modal-content1 modal-delivery">
            ...
            <div id="tab1" class="modal-delivery__tab tab-content1 active1">
                ...
                <p class="map__note">Вы можете выбрать точку на карте и если она в зоне доставки, то мы сможем привезти заказ</p>
    ...
    <p id="user_authorize" style="display:none"><?if ( is_user_logged_in() ) {echo 1;} else {echo 0;}?></p>
        ...
        <div id="time_change">
            ...
            <div class="modal-delivery__time flex_time" id="flex_time" style="">
                ...
                <style>
                    #billing_asdx1, #billing_type_delivery_sam {
                        height: 35px;
                    }
                </style>
                ...
                <script>
                    const mainblock = document.querySelector('.mainblock_time:not(.notenable)'); // получаем первый блок без класса notenable
                    if (mainblock) { // если блок найден
                    mainblock.classList.add('enable'); // добавляем класс enable
                    }

                    $('.mainblock_time').click(function() {
                    // Убрать класс 'enable' у всех элементов с классом 'mainblock_time'
                    $('.mainblock_time').removeClass('enable');
                    
                    // Добавить класс 'enable' только выбранному элементу
    ...
    function init() {
                            ...
                            if(!data.success) {
								...
								*/
                                ...
                                if($curdate > $express_date_start && $curdate < $express_date_end && ($curdate < $express_stop_time_start || $curdate > $express_stop_time_end) && 1==2) { ?>
                                    let express_checked = '';
                                    if(data.data.current == 'today_express') {
                                        express_checked = ' enable';
                                    }
                                    ...
                                    choices = choices + 
                                        '<div class="mainblock_time mainblock_time_express' + express_checked + '" data-day="today" data-time="express">' +
                                            '<div class="underblocktime" style="">' + 
                                                '<p class="delivery-text">Экспресс-доставка</p>' + 
                                                '<p>60-90 мин</p>' +
                                                '<p class="mainblock_time__price">' + express_price + '&nbsp;₽</p>' +
                                            '</div>' + 
                                        '</div>';
                                <?php } ?>
        ...
        suggestView1.events.add('select', function (e) {
            ...
            ymaps.geocode(item.value).then(function (res) {
                ...
                var firstGeoObject1 = res.geoObjects.get(0).getAddressLine();
        ...
        ymaps.geocode(coords).then(function (res) {
            ...
            let addressElem = document.getElementById('suggest1');
            if (addressElem) {
    ...
    function init() {
        ...
        suggestView1.events.add('select', function (e) {
            ...
            ymaps.geocode(item.value).then(function (res) {
                ...
                var lat = coords[0];
                var lon = coords[1];
                setMapCenter1(coords);
                if(res.geoObjects.get(0).getPremiseNumber() != null) {
                    if (firstGeoObject1.indexOf('Владивосток') !== -1 || firstGeoObject1.indexOf('Уссурийск') !== -1) {
                        if (!myPlacemark) {
                            myPlacemark = new ymaps.Placemark([lat, lon], {}, {
                                preset: 'islands#dotIcon',
                                iconColor: '#0095b6'
                        ...
                        button.classList.remove('disabled-btn');
                        button.classList.add('enable-dev');
                        document.getElementById('flex_time').style.display="flex";
                    ...
                    button.classList.add('disabled-btn');
                    deliveryMessage.style.display = 'flex';
                    var addressElem = document.getElementById('suggest1');
                    document.getElementById('flex_time').style.display="none";
                }
            });
        });
            ...
            if (document.getElementById('status_delivery').innerHTML.trim() === '') {
                navigator.geolocation.getCurrentPosition(function (position) {
                    ...
                    ymaps.geocode([lat, lon]).then(function (res) {
                        ...
                        if(res.geoObjects.get(0).getPremiseNumber() != null) {
                            if (
                            firstGeoObject1.indexOf('Владивосток') !== -1 ||
                            firstGeoObject1.indexOf('Уссурийск') !== -1
                            ) {
                                var addressElem = document.getElementById('suggest1');
                                if (addressElem) {
                                    addressElem.value = fullAddress;
                ...
                ymaps.geocode([lat, lon]).then(function (res) {
                    ...
                    if(firstGeoObject.getPremiseNumber() != null) {
                        if (address.includes('Владивосток') || address.includes('Уссурийск')) {
                            const infodev = document.getElementById('infodev');
                            infodev.innerHTML = address;
                            document.getElementById('comment_cur').style.display = 'block';
                            document.getElementById('comment_cur1').style.display = 'block';
                            document.getElementById('com_cur').style.display = 'block';
                            const button = document.getElementById('but_dev');
        ...
        // Проверяем, что координаты находятся в границах Владивостока или Уссурийска.
        ymaps.geocode(coords).then(function (res) {
            ...
            if (res.geoObjects.get(0).getPremiseNumber() != null) {
                if (
                    address.indexOf('Владивосток') !== -1 ||
                    address.indexOf('Уссурийск') !== -1
                ) {
                    deliveryMessage.style.display = 'none';
                    if (!myPlacemark) {
                    ...
                    document.getElementById('status_delivery').innerHTML = coords;
    ...
    <style>
        ...
        <div id="tab2" class="modal-delivery__tab tab-content1">
            ...
            <p>Выберите магазин, чтобы посмотреть товары в наличии и оформить заказ</p>
        ...
        <p>Выберете удобное для Вас время и мы подготовим заказ</p>
        <div class="modal-delivery__tab-selects" style="display:flex;">
            <div class="first_part_time" style="width:27%;">
                <select class="modal-delivery__tab-select" name="data" id="id_select_time_samoviviz" style="">
                    <?$current_time1 = current_time('H:i'); // Получаем текущее местное время в формате часы:минуты?>
                    <option value="today">Сегодня</option>
                    <option value="tomorrow" <?if ($current_time1 > '20:00') {echo 'selected';}?>>Завтра</option>
                </select>
            </div>
            <div class="second_part_time" style="width:73%;">
                <select name="" class="enable_time modal-delivery__tab-select" id="select_time_1" style="
                <?if ($current_time1 > '20:00') {echo 'display:none;';}?>">
                <?php
                    ...
                    $option_time = $start . '-' . $end; // Формируем строку вида "часы:минуты-часы:минуты"
                    if ($start > $current_time) { // Проверяем, если начальное время больше, чем текущее время
                        echo '<option value="' . $option_time . '">' . $option_time . '</option>'; // Выводим option
                    }
                    }
                ?>

                </select>
                <select name="" class="modal-delivery__tab-select" id="select_time_2" style="
                    <?if ($current_time1 > '20:00') {echo 'display:block !important;';}?>
                    display:none;">
                    <?php
                        $current_time = current_time('H:i'); // Получаем текущее местное время в формате часы:минуты
                        $start_time = strtotime('10:00'); // Устанавливаем начальное время
    ...
    $('#id_select_time_samoviviz').on('change', function() {
        ...
        $('#select_time_1').removeClass('enable_time');
        $('#select_time_1').hide();
        $('#select_time_2').show();
        $('#select_time_2').addClass('enable_time');

        } else {
        $('#select_time_1').addClass('enable_time');
        $('#select_time_1').show();
        $('#select_time_2').hide();
        $('#select_time_2').removeClass('enable_time');
        }
    });
    });
    </script>
    ...
    <div class="modal-content modal-delivery__selfpickup">
        ...
        <? if(1==2) : ?>
        <div id="market5" class="market_el">
            <p class="modal-delivery__selfpickup-adress">Светланская (ул. Светланская, 43)</p>
            <div class="mainblock_time1 enable1" data-market="5"  style="">
            <div class="underblocktime1" style="">
                <p class="delivery-text modal-delivery__selfpickup-btn"style="margin-bottom:0px;">Выбрать</p>
            </div>
        </div>
        ...
        <div id="market1" class="market_el">
            <p class="modal-delivery__selfpickup-adress">ТЦ «Море», Гипермаркет (ул. Некрасовская, 49а)</p>
            <div class="mainblock_time1 enable1" data-market="2" style="">
            <div class="underblocktime1" style="">
                <p class="delivery-text modal-delivery__selfpickup-btn"  style="margin-bottom:0px;">Выбрать</p>
            </div>
        </div>
        ...
        <h5 class="modal-delivery__selfpickup-city">Находка:</h5>
        <div id="market1" class="market_el">
            <p class="modal-delivery__selfpickup-adress">Находка, Проспект мира, 65/1</p>
            <div class="mainblock_time1 enable1" data-market="7" style="">
            <div class="underblocktime1" style="">
                <p class="delivery-text modal-delivery__selfpickup-btn"  style="margin-bottom:0px;">Выбрать</p>
            </div>
        </div>
    ...
    var myPlacemark2 = new ymaps.Placemark([43.127427, 131.909317], {
        hintContent: 'ТЦ «Море», Гипермаркет (ул. Некрасовская, 49а)'
    }, {
        balloonContentLayout: null
    });
    ...
    /*var myPlacemark5 = new ymaps.Placemark([43.115094047484256,131.8909049184149], {
        hintContent: 'ТЦ Светланская (Светланская, 43)'
    }, {
        balloonContentLayout: null
    });*/
    ...
    button.addEventListener('click', () => {
        ...
        if (dataMarket == 2) {
                myPlacemark2.options.set('iconColor', '#ff0000');
                document.getElementById('samoviziv').innerHTML = myPlacemark2.properties.get('hintContent');
                ymaps.geocode(myPlacemark2.geometry.getCoordinates()).then(function (res) {
                    var fullAddress = res.geoObjects.get(0).getAddressLine();
                    document.getElementById('suggest').value = fullAddress;
                    console.log(fullAddress);
                    });
                map2.setCenter(myPlacemark2.geometry.getCoordinates());
                    // Устанавливаем масштаб
                    map2.setZoom(15);
        ...
        if (dataMarket == 4) {
                myPlacemark4.options.set('iconColor', '#ff0000');
                document.getElementById('samoviziv').innerHTML = myPlacemark4.properties.get('hintContent');
                ymaps.geocode(myPlacemark4.geometry.getCoordinates()).then(function (res) {
                    var fullAddress = res.geoObjects.get(0).getAddressLine();
                    document.getElementById('suggest').value = fullAddress;
                    console.log(fullAddress);
                    });
                map2.setCenter(myPlacemark4.geometry.getCoordinates());
                    // Устанавливаем масштаб
                    map2.setZoom(15);
        ...
        if (dataMarket == 5) {
                myPlacemark5.options.set('iconColor', '#ff0000');
                document.getElementById('samoviziv').innerHTML = myPlacemark5.properties.get('hintContent');
                ymaps.geocode(myPlacemark5.geometry.getCoordinates()).then(function (res) {
                    var fullAddress = res.geoObjects.get(0).getAddressLine();
                    document.getElementById('suggest').value = fullAddress;
                    console.log(fullAddress);
                    });
        ...
        /*if (dataMarket == 7) {
                myPlacemark7.options.set('iconColor', '#ff0000');
                document.getElementById('samoviziv').innerHTML = myPlacemark7.properties.get('hintContent');
                ymaps.geocode(myPlacemark7.geometry.getCoordinates()).then(function (res) {
                    var fullAddress = res.geoObjects.get(0).getAddressLine();
                    document.getElementById('suggest').value = fullAddress;
                    console.log(fullAddress);
                });
                map2.setCenter(myPlacemark7.geometry.getCoordinates());
                map2.setZoom(15);
    ...
    var element = document.querySelector('.enable');
        var count = element.getAttribute('data-count');
        var time = element.getAttribute('data-time');
    let delivery_day = element.getAttribute('data-day');
    let delivery_time = element.getAttribute('data-time');
    ...
    // отправляем AJAX-запрос на сервер
        ...
        success: function(response) {
            // Если запрос успешно обработан, выводим сообщение об успехе
            ...
            <div class="container">
                <div class="header__inner">
                    <div class="header__desktop">
                        ...
                        <div class="header__desktop-bot">
                            <button class="header__catalog btn-green" id="catalog-desc-btn">
